...
from cffex_uda.augmentation import word_level_augment
from cffex_uda.utils import raw_data_utils
from cffex_uda.utils import tokenization


FLAGS = flags.FLAGS

flags.DEFINE_string(
    "task_name", "thuc", "The name of the task to train.")

flags.DEFINE_string(
    "raw_data_dir", None, "Data directory of the raw data")

flags.DEFINE_string(
    "lstm_model_dir", None, "Data directory of the processed data")
...
def convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    # print("type: {}, label:{}, len:{}".format(type(example.label), example.label, len(example.label)))
    # print(label_map)
    # for k,v in label_map.items():
    #     print(k,v, k==example.label)
...
def get_data_by_size_lim(train_examples, processor, sup_size):
  ...
  if(sup_size == -1):
    ...
    print("每个类别样本数量阈值：{}".format(sup_size))
    for i in range(len(train_examples)):
      label = str(train_examples[i].label)
...
def proc_single_sentence(str, max_seq_length, tokenizer, processor):
    example = raw_data_utils.InputExample(guid="0000", text_a=str, text_b=None, label="unsup")
    example.word_list_a = tokenizer.tokenize_to_word(example.text_a)
    examples = [example]
    labels = processor.get_labels()+ ["unsup"]
    features = convert_examples_to_features(
        examples, labels, max_seq_length, tokenizer,
        True, None, None)
    # print(features[0].get_dict_features())
    return features[0]
...
def main(_):
  ...
  processor = raw_data_utils.CFFEXProcessor(FLAGS.labels,FLAGS.raw_data_dir, FLAGS.sup_train_file, FLAGS.sup_dev_file, FLAGS.unsup_file)
  # Create tokenizer

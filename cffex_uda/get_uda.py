...
def get_sup_feature_specs(conf):
...
def prediction_input_fn_builder(G_input, conf, tokenizer, processor):
    def gen():
        while True:
            data = G_input.get()
            if(data == "kill_uda"):
...
class Get_UDA(object):
    def __init__(self, conf,tokenizer, processor,):
        ...
        self.conf = conf
        self.ckpt_path = self.conf['model_dir']
        ...
        self.predictor = self.estimator.predict(
            prediction_input_fn_builder(self.q,conf, tokenizer, processor),

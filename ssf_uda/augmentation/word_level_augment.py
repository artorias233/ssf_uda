...
class TfIdfWordRep(EfficientRandomGen):
  ...
  def __init__(self, token_prob, data_stats):
    ...
    # print("Top 100 in tf_idf_items:")
    # for i in range(100):
    #   print(tf_idf_items[i][0],tf_idf_items[i][1])
    self.tf_idf_keys = []
    self.tf_idf_values = []
  ...
  def get_replace_prob(self, all_words):
    ...
    # Compute the tf_idf of the current word.
  ...
  def __call__(self, example):
    ...
    if show_example:
      print("before:\n")
      for word in example.word_list_a:
        print(word,end='')
      print('\n')
  ...
  def reset_token_list(self):
    ...
    self.token_ptr = len(self.token_list) - 1
    # print("Top 100 in token_list:")
    # for i in range(100):
    #   print(self.token_list[i], end=" ")
    # tf.logging.info("sampled token list: {:s}".format(
    #     " ".join(self.token_list)))

def eda(examples, aug_ops, sub_set, aug_copy_num,
                     start, end, data_total_size):
    tf.logging.info("\nstart:\n{},\nend:\n{}\ndata_total_size{}\nexamples:\n{}".format(
        start,end,data_total_size,examples))

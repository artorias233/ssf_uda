...
from utils import raw_data_utils, tokenization
import  uda
...
def save_model(ckpt_path):
    path_list = os.path.split(ckpt_path)
    root = path_list[0]
    dst_dir = os.path.join(path_list[0],"best_model")
    if tf.gfile.Exists(dst_dir):
        shutil.rmtree(dst_dir)
    tf.gfile.MakeDirs(dst_dir)
    file_list = os.listdir(root)
    for file in file_list:
        if path_list[1] in file:
...
def main(_):
  ...
  if FLAGS.do_train and FLAGS.do_eval:
    ...
    for _ in range(0, FLAGS.num_train_steps, save_checkpoints_steps):
      ...
      if dev_result["eval_classify_accuracy"]>best_acc:
        best_acc = dev_result["eval_classify_accuracy"]
        best_step = _
        best_model_name = "model.ckpt-{}".format(best_step)
        save_model(os.path.join(FLAGS.model_dir, best_model_name))
    ...
    eval_info = processor.get_eval_info()
    best_acc = 0
    best_result = {}
    best_model = ""
    for ckpt_path in checkpoint_state.all_model_checkpoint_paths:
      ...
      print_metric(ckpt_path, eval_info, dev_result)
    print_metric(best_model,  eval_info, best_result)
